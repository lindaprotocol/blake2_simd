...
const BLAKE2B_MAX_LEN: usize = 3 * blake2b_rfc::BLOCKBYTES;
const BLAKE2B_MAX_N: usize = 2 * blake2b_rfc::many::MAX_DEGREE;

fn random_params_blake2b(rng: &mut rand_chacha::ChaChaRng) -> blake2b_rfc::Params {
    ...
    params.hash_length(rng.gen_range(1, blake2b_rfc::OUTBYTES + 1));
    if rng.gen() {
        let len: usize = rng.gen_range(1, blake2b_rfc::KEYBYTES + 1);
        let key_buf = &[1; blake2b_rfc::KEYBYTES];
...
fn with_random_inputs_blake2b(mut f: impl FnMut(&[blake2b_rfc::Params], &[&[u8]])) {
    ...
    for _ in 0..NUM_TESTS {
        ...
        let mut params = ArrayVec::<blake2b_rfc::Params, BLAKE2B_MAX_N>::new();
...
fn fuzz_blake2b_hash_many() {
    with_random_inputs_blake2b(|params, inputs| {
        ...
        let mut expected = ArrayVec::<blake2b_rfc::Hash, BLAKE2B_MAX_N>::new();
        ...
        // the same result.
        let mut jobs: ArrayVec<blake2b_rfc::many::HashManyJob, BLAKE2B_MAX_N> = inputs
            .iter()
            .zip(params.iter())
            .map(|(input, param)| blake2b_rfc::many::HashManyJob::new(param, input))
            .collect();
...
fn fuzz_blake2b_update_many() {
    with_random_inputs_blake2b(|params, inputs| {
        ...
        // Now compute the same hashes in a batch, and check that this gives
        // the same result.
        let mut states = ArrayVec::<blake2b_rfc::State, BLAKE2B_MAX_N>::new();
        for param in params {
            states.push(param.to_state());
        }
        blake2b_rfc::many::update_many(states.iter_mut().zip(inputs.iter()));
        blake2b_rfc::many::update_many(states.iter_mut().zip(inputs.iter()));
...
const BLAKE2S_MAX_LEN: usize = 3 * blake2s_rfc::BLOCKBYTES;
const BLAKE2S_MAX_N: usize = 2 * blake2s_rfc::many::MAX_DEGREE;

fn random_params_blake2s(rng: &mut rand_chacha::ChaChaRng) -> blake2s_rfc::Params {
    let mut params = blake2s_rfc::Params::new();
    // hash_length, key, and last_node are all things that need to be passed
    // from the Params through to the State or whatever. Randomize those.
    // Everything else just winds up in the state words and doesn't really need
    // to be exercised here.
    params.hash_length(rng.gen_range(1, blake2s_rfc::OUTBYTES + 1));
    if rng.gen() {
        let len: usize = rng.gen_range(1, blake2s_rfc::KEYBYTES + 1);
        let key_buf = &[1; blake2s_rfc::KEYBYTES];
        params.key(&key_buf[..len]);
    }
    params.last_node(rng.gen());
    params
...
fn with_random_inputs_blake2s(mut f: impl FnMut(&[blake2s_rfc::Params], &[&[u8]])) {
    let mut rng = rand_chacha::ChaChaRng::seed_from_u64(0);
    ...
    for _ in 0..NUM_TESTS {
        ...
        // For each input slice, create a random Params object.
        let mut params = ArrayVec::<blake2s_rfc::Params, BLAKE2S_MAX_N>::new();
        for _ in 0..num_inputs {
...
fn fuzz_blake2s_hash_many() {
    with_random_inputs_blake2s(|params, inputs| {
        // Compute the hash of each input independently.
        let mut expected = ArrayVec::<blake2s_rfc::Hash, BLAKE2S_MAX_N>::new();
        for (param, input) in params.iter().zip(inputs.iter()) {
            expected.push(param.hash(input));
        }
        ...
        // Now compute the same hashes in a batch, and check that this gives
        // the same result.
        let mut jobs: ArrayVec<blake2s_rfc::many::HashManyJob, BLAKE2S_MAX_N> = inputs
            .iter()
            .zip(params.iter())
            .map(|(input, param)| blake2s_rfc::many::HashManyJob::new(param, input))
            .collect();
...
fn fuzz_blake2s_update_many() {
    with_random_inputs_blake2s(|params, inputs| {
        ...
        // Now compute the same hashes in a batch, and check that this gives
        // the same result.
        let mut states = ArrayVec::<blake2s_rfc::State, BLAKE2S_MAX_N>::new();
        for param in params {
            states.push(param.to_state());
        }
        blake2s_rfc::many::update_many(states.iter_mut().zip(inputs.iter()));
        blake2s_rfc::many::update_many(states.iter_mut().zip(inputs.iter()));
        for i in 0..states.len() {
